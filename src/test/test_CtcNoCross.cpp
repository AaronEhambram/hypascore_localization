...
int measure_x_max = 400.0;
int measure_x_slider = 247.0;
int measure_y_max = 400.0;
int measure_y_slider = 247.0;
...
void sivia(codac::CtcNoCross& ctc, ibex::IntervalVector pose, ibex::IntervalVector pm, ibex::IntervalVector a, ibex::IntervalVector b, std::vector<ibex::IntervalVector>& intermediate_result)
{
  std::queue<ibex::IntervalVector> q;
  q.push(pose); 
  ibex::IntervalVector pose_i(2),pm_i(2),a_i(2),b_i(2);
  ...
  while(!q.empty())
  {
    ...
    pm_i = pm; a_i = a; b_i = b; 
    q.pop();
    ctc.contract(pose_i,pm_i,a_i,b_i);
    if(!pose_i.is_empty())
    {
...
static void on_trackbar( int, void* )
{
  ...
  ibex::IntervalVector pm(2); 
  pm[0] = (double)(measure_x_slider-200)/10.0; 
  pm[1] = (double)(measure_y_slider-200)/10.0; 
  pm[0].inflate(0.05);
  pm[1].inflate(0.05);
  ...
  draw_box(im,pose+pm,hull,pixel_per_m,cv::Scalar(0,0,255),-1);
  ...
  cv::imshow("No Cross Contractor",im);
}

int main(int argc, char **argv)
{
  cv::namedWindow("No Cross Contractor", cv::WINDOW_AUTOSIZE);

  char TrackbarName[50];
  snprintf( TrackbarName, sizeof(TrackbarName), "x-position x %f", 0.1 );
  cv::createTrackbar( TrackbarName, "No Cross Contractor", &pose_x_slider, pose_x_max, on_trackbar );

  char TrackbarName1[50];
  snprintf( TrackbarName1, sizeof(TrackbarName1), "y-position x %f", 0.1 );
  cv::createTrackbar( TrackbarName1, "No Cross Contractor", &pose_y_slider, pose_y_max, on_trackbar );

  char TrackbarName2[50];
  snprintf( TrackbarName2, sizeof(TrackbarName2), "x-dist measure x %f", 0.1 );
  cv::createTrackbar( TrackbarName2, "No Cross Contractor", &measure_x_slider, measure_x_max, on_trackbar );

  char TrackbarName3[50];
  snprintf( TrackbarName3, sizeof(TrackbarName3), "y-dist measure x %f", 0.1 );
  cv::createTrackbar( TrackbarName3, "No Cross Contractor", &measure_y_slider, measure_y_max, on_trackbar );
